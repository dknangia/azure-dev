...
public static class DataProcessingManager
{
    ...
    public static int CalculateDistance(double dealerLatitude, double dealerLongitude)
    {
        ...
        const double x1 = Latitude * (Math.PI / 180.0);
        const double y1 = Longitude * (Math.PI / 180.0);
        var x2 = dealerLatitude * (Math.PI / 180.0);
        var y2 = dealerLongitude * (Math.PI / 180.0) - y1;
